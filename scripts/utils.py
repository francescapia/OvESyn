...
def add_data_dir2path(list_files: list, data_dir: str, fold: int = None, args = None, index = None, phase = None) -> tuple[list, list]:
    ...
    for d in new_list_files:
        ...
        if args and args.embedding_base_dir is not None:
            base_data_dir = getattr(args, "data_base_dir", None)
            image_path = d["image"]
            if base_data_dir:
                image_path = image_path.replace(base_data_dir, args.embedding_base_dir)

            d["image"] = image_path
            str_info = image_path + ".json"
